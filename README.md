...
> 压缩是有损的。所以在压缩“扣动扳机”之前，还有一道 **Memory Flush（压缩前记忆刷写）**——先让模型把值得长期保存的信息写入长期记忆，再压缩。详见下方 Memory 小节。
...
除检索外，Memory 还提供 `memory_save`（写入）和 `memory_get`（按 ID 拉取全文），由模型自主决定何时存取：先 `memory_search` 命中、再 `memory_get` 拉细节；遇到值得长期保存的信息（用户偏好、关键决策）才 `memory_save`。

**压缩前记忆刷写（Memory Flush）**：长对话触发 compaction 时摘要是有损的、可能丢细节。为此在压缩之前插入一个独立回合——当 token 逼近压缩阈值（提前一个 softThreshold）时，用 flush 提示词让模型把 durable 信息 `memory_save` 落盘，再压缩。这样跨会话的关键记忆不会被摘要冲掉，是“有损操作前的无损保全”模式。对应 openclaw 的 `auto-reply/reply/memory-flush.ts`。
...
| 压缩前记忆刷写 | `memory-flush.ts` | token 逼近压缩阈值前，让模型落盘 durable 记忆 |
| thinking 持久化 | `agent-loop.ts` | 提取含 signature 的 thinking block 回传（Anthropic 多轮 tool use 约束） |
| 子代理派生 | `agent.ts` | sessions_spawn 后台跑子任务，事件流回传摘要 |
